...
tp @s[tag=shp_right_door] ^0.0444444444 ^ ^
tp @s[tag=shp_left_door] ^-0.0444444444 ^ ^

execute if score @s door_tick matches 18.. run tag @s remove shp_open
execute if score @s[tag=shp_right_door] door_tick matches 18 at @s as @a[distance=..20] at @s run playsound minecraft:block.netherite_block.place block @s ^ ^ ^ 3 0
...
execute if score @s[tag=shp_right_door] door_tick matches 18 at @s run tag @e[type=minecraft:area_effect_cloud,sort=nearest,limit=1,tag=exit_door] remove shp_opening